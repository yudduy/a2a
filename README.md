...
Open Deep Research provides two different implementations for report generation:

1. **Graph-based Implementation** (`src/open_deep_research/graph.py`): The original implementation that uses a plan-and-execute workflow with human feedback on the report plan. This implementation provides a more interactive experience where users can approve or modify the report plan before generating content.

2. **Multi-agent Implementation** (`src/open_deep_research/multi_agent.py`): A newer parallel implementation that uses a supervisor agent to manage multiple researcher agents. Each researcher agent independently handles a specific section of the report. This implementation enables faster report generation with parallel processing of sections.

### Testing Report Quality

To test and compare the quality of reports generated by both implementations:

```bash
cd /Users/rlm/Desktop/Code/open_deep_research
source open-deep-research-env/bin/activate

# Test the multi-agent implementation
LANGSMITH_TRACING_V2=true LANGSMITH_PROJECT="Open Deep Research: Report Quality" ./tests/run_test.py --agent multi_agent

# Test the graph-based implementation
LANGSMITH_TRACING_V2=true LANGSMITH_PROJECT="Open Deep Research: Report Quality" ./tests/run_test.py --agent graph

# Test both implementations
LANGSMITH_TRACING_V2=true LANGSMITH_PROJECT="Open Deep Research: Report Quality" ./tests/run_test.py --all
```

The test results will be logged to LangSmith, allowing you to compare the quality of the reports generated by each implementation.
